 ...
 * @param {string} [options.cwd=process.cwd()] - Current working directory for ${CWD} expansion
...
function resolveMounts(config, options = {}) {
   ...
   * @param {'host'|'container'} type - Path type (host resolves ${CWD}, container resolves $HOME)
   * @returns {string}
   */
  function resolvePath(path, type) {
    if (type === 'host') {
      return path.replace(/\$\{CWD\}/g, cwd);
    }
    return path.replace(/\$HOME/g, containerHome);
  }
  ...
  return config.flatMap((item) => {
    if (typeof item === 'string') {
      ...
      return {
        host: resolvePath(preset.host, 'host'),
        container: resolvePath(preset.container, 'container'),
    ...
    if (typeof item === 'object' && item !== null) {
      ...
      return {
        host: resolvePath(item.host, 'host'),
        container: resolvePath(item.container, 'container'),
